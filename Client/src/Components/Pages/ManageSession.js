...
axios.get('http://localhost:3000/sessions',{
  ...
  var newarr=response.data.sessionlist.map(obj => ({ ...obj, id: obj._id }))
console.log(newarr);
...
const updateProgram=()=>{
   ...
   axios.patch('http://localhost:3000/sessions/updatesession/'+selectedobj._id,obj,{
    headers: {
        ...
        <>
          <Button
            onClick={() => {
              ...
              axios.delete('http://localhost:3000/sessions/deletesession/'+props.row.id,{
                headers: {
                  Authorization: `Bearer ${gettoken}`,
                },
              }).then((response) => {
...
<DataTable header={sessionHeader} data={sessionlist}/></div>
</div>
  );
}
